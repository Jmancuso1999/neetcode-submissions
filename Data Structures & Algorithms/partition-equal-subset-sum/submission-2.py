class Solution:
    # Memoization (Top Down) - O(n * target) time and space
    def canPartition(self, nums: List[int]) -> bool:
        ...
        for pos in range(len(nums) + 1):
            dp[pos][0] = True # At target == 0 and at ANY row since target is 0 its automatically True

        # Typical knapsack formula
        for pos in range(1, len(nums) + 1):
            for tgt in range(1, target + 1):
                if nums[pos - 1] <= tgt:
                    dp[pos][tgt] = dp[pos - 1][tgt] or dp[pos - 1][tgt - nums[pos - 1]]
                else:
                    dp[pos][tgt] = dp[pos - 1][tgt]
...
class Solution:
    ...
    def canPartition(self, nums: List[int]) -> bool:
        ...
        def dfs(pos, target):
            ...
            # At the end we end up back at the starting pos / target so check that value when we return
            return dp[pos][target]
